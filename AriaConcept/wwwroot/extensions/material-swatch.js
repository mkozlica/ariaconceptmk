class MaterialSwatchExtension extends Autodesk.Viewing.Extension {
    constructor(viewer, options) {
        ...
        this._swatch = null;
        //this._urn = 'dXJuOmFkc2sub2JqZWN0czpvcy5vYmplY3Q6YmR5YWE4dWlqZmpjbDJ2a2hvZ2Yzc3ZuYXVtcXN2a2QtYmFzaWNtb2RlbHMvY29sb3JiaWcuemlw'; // 0 deg
        this._urn = 'dXJuOmFkc2sub2JqZWN0czpvcy5vYmplY3Q6YmR5YWE4dWlqZmpjbDJ2a2hvZ2Yzc3ZuYXVtcXN2a2QtYmFzaWNtb2RlbHMvY29sb3JiaWc5MC56aXA';// 90 deg
    }
    ...
    async applyPresetOnList(name, targetModel, listOfTargetObjectId) {
        const presets = await this.getPresets();
        if (!presets.has(name)) {
            console.error('Material swatch not found', name);
            return;
        ...
        const tree = targetModel.getInstanceTree();
        const frags = targetModel.getFragmentList();
        listOfTargetObjectId.forEach(targetObjectId => {
            tree.enumNodeFragments(targetObjectId, function (fragid) {
                frags.setMaterial(fragid, material);
            }, true);
        });
